...
| Yelp signal | `src/itinerary_system/utility_model.py` | If Yelp is present, uses normalized Yelp evidence; if Yelp is unavailable, the Yelp utility term is masked out of that POI denominator rather than scored as zero. | Yelp columns or explicit source mask. | Popularity/quality proxy when available. | Missing Yelp is unknown, while present-but-poor Yelp can still lower utility. |
| Source coverage / data confidence alias | `src/itinerary_system/utility_model.py`; `src/itinerary_system/data_enrichment.py` | Explicit `has_*` masks compute `source_coverage_score`; legacy `data_confidence` mirrors coverage only for compatibility. | Source flags and enrichment coverage. | Source coverage score. | Not a calibrated uncertainty or utility term. |
...
| Current recomputed final POI value | `src/itinerary_system/data_enrichment.py` | Enrichment emits source masks and coverage before utility scoring; the utility model then uses masked denominators for reportable scores. | Enrichment pipeline and utility model. | Current enriched POI value before utility model selection. | Still a heuristic proxy; source ablation should be used for provenance claims. |
